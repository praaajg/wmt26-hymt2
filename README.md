...
| Path | Description |
|------|-------------|
| `scripts/Hy-MT2-1_8B_train_r16_α32.ipynb` | QLoRA training notebook |
| `scripts/evaluate_hymt2_tvsub.ipynb` | Inference & evaluation on TVsub test set |
| `scripts/inference.ipynb` | SRT subtitle translation inference (line-independent, no metadata conditioning; preserves indices/timestamps) |
| `scripts/inference_with_metadata.ipynb` | Variant SRT inference that injects per-video title/album-title/synopsis from `*_metadata.json` into the prompt (alternative submission design) |
...
| `test_results/test_hypotheses.txt` | Model translations on TVsub test set (200 pairs) |
| `test_results/test_hypotheses.metrics.json` | Evaluation metrics |
| `tvsub/` | TVsub dataset (original + processed splits) |
...
| **Data** | TVsub processed split — 60K train / 500 dev / 200 test pairs |
...
```bash
git clone https://github.com/praaajg/wmt26-hymt2
cd wmt26-hymt2
```
...
model = PeftModel.from_pretrained(base_model, "wmt26-hymt2")
model.eval()
...
Metrics computed by `scripts/evaluate_hymt2_tvsub.ipynb` using sacrebleu (tokenize="13a", smooth="exp") for sacreBLEU/chrF/TER and HuggingFace evaluate for BLEU.

---

## SRT Inference

Two inference pipelines are provided; they share the same fine-tuned adapter, decoding settings, and SRT round-trip handling, and differ only in prompt conditioning:

- **`scripts/inference.ipynb` (submitted pipeline, no-metadata version).** Each block of a `*_zh.srt` file is embedded in the training prompt template and translated independently, with no per-video metadata conditioning. Blocks with empty source text are passed with a placeholder single-space input but written back as empty strings, so empty subtitle timings are preserved without hallucinated content. Batches of 16 blocks are decoded with the generation settings above, the first output line is kept, and results are written back with the original indices and timestamps unaltered.
- **`scripts/inference_with_metadata.ipynb` (alternative variant).** Same as above, except each subtitle line is wrapped in a context block populated from the accompanying `*_metadata.json` record (video title, album title, album-level description), and empty blocks are written back as a single space. Expects a test-set folder with `*_zh.srt` and `*_metadata.json` files. This path was evaluated only qualitatively (no reference translations exist for the official test set), so its effect on aggregate quality is unmeasured.
